...
import { getNavamsaChart, getBirthChart } from "./position.js";
import {
  calculateNakshatra,
 ...
 */
export function areCompatibile(
  person1,
  person2,
 ...
 */
export function getHousesOfChart(chart) {
...
export function getReverseHousesOfChart(houses) {
  const reverseHouses = {};
...
export function seventhHouseOfD9Check(navamsaChart1, birthChart2) {
...
export function oppositeSignOfBirthCheck(birthChart1, birthChart2) {
...
export function areFriends(planet1, planet2) {
  ...
  if (isNeutral >= 0) return 1;
  return 0;
}

export function connectionType(sign1, sign2) {
  ...
  if (constants.GOOD_CONNECTION_TYPES.indexOf(result) >= 0) {
    return 1; // point
  }
  return 0;
}

export function signInterference(birthChart1, birthChart2, planet) {
  const firstPersonHouses = getHousesOfChart(birthChart1);
...
export function planetConjunctHouse(birthChart1, birthChart2, planet, house) {
...
export function checkManglikConnection(birthChart1, birthChart2) {
  const person1Houses = getHousesOfChart(birthChart1);
  const person2Houses = getHousesOfChart(birthChart2);
  ...
  if (constants.MANGLIK.indexOf(Number(person1MangalBhava)) >= 0) {
    if (constants.MANGLIK.indexOf(Number(person2MangalBhava)) >= 0) {
      // Both Manglik
      return 1;
    }
    // One is Manglik
...
export function checkPlanetCorrespondenceOfPlanetInSecondChart(
  birthChart1,
  birthChart2,
  planet
) {
